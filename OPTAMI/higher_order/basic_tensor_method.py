...
class BasicTensorMethod(Optimizer):
    ...
    def step(self, closure, hess=True, steps_every=1):
        """Solves a subproblem.
        Arguments:
        ...
        for group in self.param_groups:
            ...
            if subsolver is None:
                if not hess:
                    full_hessian = self.cached_hess
                else:
                    full_hessian = derivatives.flat_hessian(df, list(params)).to(torch.double)
                    self.cached_hess = full_hessian
                    self.eigenvalues, self.eigenvectors = torch.linalg.eigh(full_hessian)
            ...
            for i in range(max_iters_outer):
                ...
                with torch.no_grad():
                    D3xx = D3xx.to(torch.double).cuda()
                    Hx = Hx.to(torch.double).cuda()
                ...
                if subsolver is None:
                    v_flat = exact(L, c.detach(), T=self.eigenvalues, U=self.eigenvectors)
                else:
